...
<head>
    ...
    <meta name="description" content="Kampung Wisata Kauman Yogyakarta - Event, Paket Wisata, Artikel, dan Galeri Foto & Video">
...
<body class="bg-gray-50">
    ...
    <?php if (!empty($event_slider)): ?>
        <section class="relative h-[380px] md:h-[520px] overflow-hidden">
                    ...
                    <div class="min-w-full relative bg-cover bg-center"
                        ...
                        <div class="relative h-full flex items-center">
                            <div class="max-w-7xl mx-auto px-6 text-white">
                                <span class="inline-block mb-3 px-3 py-1 text-xs bg-white/20 rounded-full">📅 Event</span>
                                ...
                                <div class="flex items-center gap-4 text-sm text-gray-300">
                                    <span>📍 <?= htmlspecialchars($e->lokasi) ?></span>
                                    <span>•</span>
            ...
            <div class="grid md:grid-cols-2 gap-12 items-center">
                <div>
                    ...
                    <p class="text-gray-600 line-clamp-3">
                        Kauman dikenal sebagai living museum...
                    </p>
                    <a href="<?= site_url('user/tentang_kami') ?>" class="inline-block mt-6 text-orange-600 font-medium hover:underline">
                        Selengkapnya →
                    </a>
                </div>
                <div class="relative">
    ...
    <section class="py-20 reveal bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
            <div class="text-center mb-12">
                <h2 class="text-3xl font-bold text-gray-800 mb-3">Paket Wisata Unggulan</h2>
                <p class="text-gray-600">Paket yang paling sering dipesan pengunjung</p>
            </div>
                            ...
                            <div class="p-6">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded">
                                        <?= intval($p->total_dipesan) ?>x dipesan
                                    </span>
                                </div>
                            </div>
                        </a>
            ...
            <?php else: ?>
                <p class="text-center italic text-gray-500">Belum ada paket unggulan.</p>
            <?php endif; ?>
        </div>
    </section>
<!-- ================= DAFTAR PAKET WISATA ================= -->
...
<section class="py-20 bg-white reveal">
        ...
        <?php if (!empty($artikel)): ?>
            <div class="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-4">
                ...
                <?php foreach ($artikel as $a): ?>
                    <?php if ($a->status !== 'publish') continue; ?>
    ...
    <div class="max-w-7xl mx-auto px-6 relative">
        ...
        <div class="relative">
            <!-- Tombol kiri -->
            <button id="scrollLeft" class="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-20 
                w-10 h-10 bg-white shadow rounded-full items-center justify-center hover:bg-gray-100">
                ◀
            </button>

            <!-- Tombol kanan -->
            <button id="scrollRight" class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-20 
                w-10 h-10 bg-white shadow rounded-full items-center justify-center hover:bg-gray-100">
                ▶
            </button>

            <!-- Scrollable gallery -->
            <div id="galleryScroll" class="flex gap-4 overflow-x-auto scroll-smooth pb-4 snap-x snap-mandatory">
                <?php foreach ($galeri as $g): ?>
                    <div class="min-w-[280px] max-w-[280px] snap-start
                        bg-white rounded-xl shadow-sm cursor-pointer gallery-item flex-shrink-0"
                        data-type="<?= $g->jenis ?>"
                        data-src="<?= base_url('uploads/galeri/' . $g->file) ?>"
    ...
    btnLeft?.addEventListener("click", () => gallery.scrollBy({ left: -scrollAmount, behavior: 'smooth' }));
    btnRight?.addEventListener("click", () => gallery.scrollBy({ left: scrollAmount, behavior: 'smooth' }));
    ...
    document.querySelectorAll('.gallery-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            modalContent.innerHTML = type === 'foto' 
                ? `<img src="${src}" class="w-full max-h-[70vh] object-contain bg-black">`
                : `<video controls autoplay class="w-full max-h-[70vh] bg-black"><source src="${src}" type="video/mp4"></video>`;
    ...
    if (routeItems.length && window.IntersectionObserver && window.anime) {
        const observer = new IntersectionObserver(entries => {
            entries.forEach(entry => {
                ...
                line && anime({ targets: line, height: ['0px','100%'], duration:600, easing:'easeOutQuad' });
                anime({ targets: dot, scale:[0,1], opacity:[0,1], duration:500, delay:150, easing:'easeOutBack' });
                anime({ targets: card, translateX:[40,0], opacity:[0,1], duration:600, delay:250, easing:'easeOutExpo' });
